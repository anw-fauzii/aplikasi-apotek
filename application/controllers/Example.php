...
class Example extends CI_Controller
{
	...
	private function check_admin() {
        if (!$this->session->userdata('logged_in') || $this->session->userdata('role') != 'admin') {
            show_error('You are not authorized to access this page.', 403); // Show 403 Forbidden error
        }
    }
	...
	function table_invoice() {
		...
		if($this->input->post('tanggal_awal')){
			$tanggal_awal = date('d F Y', strtotime($this->input->post('tanggal_awal')));
			$tanggal_akhir = date('d F Y', strtotime($this->input->post('tanggal_akhir')));
		...
		if($data['tanggal_awal'] == $data['tanggal_akhir']){
			$this->template->write('title', 'Data Penjualan '.$data['tanggal_awal'], TRUE);
		}elseif($data['tanggal_awal'] && $data['tanggal_akhir']){
			$this->template->write('title', 'Data Penjualan '.$data['tanggal_awal'].' - '.$data['tanggal_akhir'], TRUE);
		}elseif($data['tanggal_awal'] == NULL && $data['tanggal_akhir'] == NULL){
			$this->template->write('title', 'Data Penjualan', TRUE);
		}
		$this->template->write('header', 'Sistem Informasi Apotek');
